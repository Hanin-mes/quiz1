...
Answer: gitignore file tells git which files to ignore when commiting to your project to the github repository so that .*txt will not be pushed to the remote.
...
``git checkout v2.0``
Answer: Tag v2.0 on b so that the head will be on b.
...
Answer:no since this command for the modified or removed files.
